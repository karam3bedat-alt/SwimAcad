    ...
    const generateFallbackInsights = () => {
      ...
      if (pendingPayments.length > 0) {
        ...
        insights.push({
          ...
          desc: `هناك ${pendingPayments.length} دفعات معلقة بقيمة إجمالية ${totalDue} شيكل. تشمل: ${names}.`,
          type: 'warning',
          category: 'warning',
    ...
    const tryGenerate = async (): Promise<void> => {
      try {
        const prompt = `
          ...
          2. تفصيل الدفعات غير المكتملة أو المعلقة (Pending Payments) مع ذكر أسماء الطلاب والمبالغ المتبقية بالعملة المحلية (شيكل).
          ...
          قواعد هامة:
          - يجب أن يكون الوصف (desc) غنياً بالتفاصيل والأسماء (مثل: "الطالب أحمد لديه دفعة معلقة بقيمة 50 شيكل").
          - اجعل الرد شخصياً ومهنياً وموجهاً لمدير الأكاديمية.
          - استخدم العملة شيكل أو ₪.
